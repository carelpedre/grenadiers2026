// Liste ISO 3166-1 (noms français + code alpha-2).
// Utilisée par le sélecteur de pays du Mur des supporters.
// PINNED_CODES épingle les pays de la diaspora en tête du menu.

export const PINNED_CODES = ["HT", "US", "CA", "FR", "DO", "CL", "BR"];
...
// Pays épinglés (dans l'ordre PINNED_CODES) puis le reste trié alphabétiquement.
export function orderedCountries() {
  const byCode = Object.fromEntries(countries.map((c) => [c.code, c]));
  const pinned = PINNED_CODES.map((code) => byCode[code]).filter(Boolean);
